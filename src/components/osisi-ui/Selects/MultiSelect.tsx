...
export function MultiSelect({
  ...
  const groupedOptions = options.reduce(
    ...
    {} as Record<string, MultiSelectOptionsType[]>
  );
    ...
    <Popover open={open} onOpenChange={setOpen} {...props}>
      <PopoverTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className="flex py-6 px-4 border border-accent min-h-[100px] rounded-lg"
          disabled={disabled}
        >
          <div className="flex gap-1 flex-wrap overflow-x-auto max-h-[10rem] items-center h-full">
            {selected.length > 0 ? (
              selected.map((option) => (
                <Badge
                  key={option}
                  className="bg-secondary/5 text-secondary shadow-none border border-secondary/20 rounded capitalize"
                >
                  {option}
                  <div
      ...
      <PopoverContent className="w-full p-0 rounded">
        <Command className="w-full rounded font-body bg-background">
          <CommandInput
